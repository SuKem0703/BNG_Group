...
public class MapMove : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (isInternalMove)
        {
            StartCoroutine(InternalMoveRoutine());
            return;
        }
        ...
        if (SaveController.Instance != null)
        {
            ...
            SaveController.Instance.SaveGame(SaveReason.SceneTransition, (isSuccess) =>
            {
                if (isSuccess)
                {
                    if (Unity.Netcode.NetworkManager.Singleton != null && Unity.Netcode.NetworkManager.Singleton.IsListening)
    ...
    private IEnumerator InternalMoveRoutine()
    {
        ...
        {
            PlayerCore.Instance.SetPosition(playerPosition);
        }
